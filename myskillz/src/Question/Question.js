...
class Question extends React.Component{
    ...
    render(){
        ...
        for(let i = 1; i <= this.state.rating; i++){
            startsArray.push(<i key={i} onClick={() => this.onStarClicked(i)} className="question__star fa-solid fa-star"></i>)
        }
        for(let i = this.state.rating + 1; i <= 5; i++){
            startsArray.push(<i key={i} onClick={() => this.onStarClicked(i)} className="question__star fa-regular fa-star"></i>)
        }
